#!/usr/bin/env bash
# ==============================================================================
# Minecraft Firewall Controller (Zero-Downtime Dynamic Shield Manager)
...
function enable_shield() {
    ...
    # 2. Rate Limiting based on mode (Only affects NEW connection attempts; NEVER drops existing players)
    if [[ "$mode" == "strict" ]]; then
        # Strict mode (During active massive bot raids): 10 conns/sec per IP
        ...
        # Normal mode: 25 conns/sec per IP
    ...
    # 3. Hook filter chains to game ports
    ...
    echo "[✓] Minecraft Firewall Shield is now ENABLED ($mode mode). Existing players remain connected!"
}

function disable_shield() {
    check_root
    echo "[+] Disabling Minecraft Firewall Shield..."
    iptables -D INPUT -i "$DEFAULT_IFACE" -p tcp -m multiport --dports 25565:25600,30000:40000 -j MC_TCP_FILTER 2>/dev/null || true
    iptables -D INPUT -i "$DEFAULT_IFACE" -p udp -m multiport --dports 25565:25600,30000:40000 -j MC_UDP_FILTER 2>/dev/null || true
    iptables -F MC_TCP_FILTER 2>/dev/null || true
    iptables -F MC_UDP_FILTER 2>/dev/null || true
    echo "[✓] Minecraft Firewall Shield is now DISABLED. All traffic passes directly."
}

function show_status() {
    echo "=========================================================="
    echo " Minecraft Firewall & Anti-DDoS Status"
    echo "=========================================================="
